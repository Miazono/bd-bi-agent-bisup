...
# Логирование помогает читать этот скрипт как пошаговый конвейер.
logging.basicConfig(
...
# Общие значения конфигурации слоя bronze.
...
# Жёстко заданная раскладка raw-файлов в MinIO.
# Это контракт между load_raw.py и load_bronze.py.
RAW_LAYOUT = {
...
# Схема временных внешних таблиц.
# Все колонки сначала читаются как VARCHAR, а приведение типов делается в SQL.
RAW_TABLE_COLUMNS = {
    "hm_articles_raw": [
        "article_id",
        "product_code",
...
def q(value: str) -> str:
    # Минимальное экранирование строк для SQL-подстановок.
    return value.replace("'", "''")


def bronze_ddl_paths() -> List[Path]:
    # Физическая модель слоя bronze создаётся отдельно от логики наполнения.
...
def bronze_query_paths() -> Dict[str, Path]:
    # Вспомогательная карта SQL-файлов слоя bronze.
    # Сейчас она не участвует в main(), но полезна для навигации и расширения.
...
def validate_raw_files(s3: S3Client, bucket: str, raw_prefix: str, load_date: str) -> Dict[str, str]:
    # Проверяем, что нужные raw-файлы уже лежат в MinIO.
    resolved = {}
...
def create_schemas(trino: TrinoClient, bucket: str) -> None:
    # Создаём схему для временных external tables и схему для физических таблиц bronze.
...
def create_raw_external_table(
    ...
    # Все колонки external table объявляем как VARCHAR:
    # это переносит приведение типов в SQL-загрузчики bronze.
...
def create_all_raw_tables(
    ...
    # Для каждого исходного CSV создаём отдельную внешнюю таблицу.
...
def create_bronze_tables(trino: TrinoClient) -> None:
    # Физические таблицы bronze создаются из DDL-файлов.
...
def delete_batch(trino: TrinoClient, batch_id: str) -> None:
    # Повторный прогон того же batch должен быть идемпотентным.
    batch = q(batch_id)
...
def load_articles_to_bronze(trino: TrinoClient, batch_id: str, source_file_name: str) -> None:
    # Подставляем параметры в SQL-файл и выполняем его через Trino.
    batch = q(batch_id)
...
def log_counts(trino: TrinoClient, batch_id: str) -> None:
    # Базовая техническая проверка после загрузки: сколько строк попало в каждую таблицу.
    batch = q(batch_id)
...
def parse_args() -> argparse.Namespace:
    # CLI сделан минимальным: дата raw и batch_id.
...
def main() -> None:
    # Весь сценарий bronze-загрузки идёт строго по шагам.
    args = parse_args()
