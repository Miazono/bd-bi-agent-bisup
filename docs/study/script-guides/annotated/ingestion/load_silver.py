...
# Базовая настройка логирования для пошагового контроля загрузки.
logging.basicConfig(
...
# Общие параметры окружения, которые нужны почти в каждом SQL-шаге.
LAKEHOUSE_BUCKET = settings.lakehouse_bucket
SILVER_PREFIX = settings.silver_prefix

# Помощник для поиска SQL-файлов внутри репозитория.
SQL_ASSETS = SqlAssets()


def q(value: str) -> str:
    # Простое экранирование одинарной кавычки для безопасной подстановки в SQL.
    return value.replace("'", "''")


def silver_ddl_paths() -> List[Path]:
    # Явный список DDL-файлов слоя silver.
...
def silver_query_paths() -> Dict[str, Path]:
    # Карта SQL-файлов, которые наполняют или обновляют таблицы silver.
...
def execute_step(trino: TrinoClient, sql: str, title: str) -> None:
    # Единый шаблон логирования для всех SQL-операций.
...
def log_row_count(trino: TrinoClient, table_name: str) -> None:
    # Простая техническая проверка: сколько строк сейчас лежит в таблице.
...
def parse_fqn(table_name: str) -> tuple[str, str, str]:
    # Разбираем имя вида catalog.schema.table, чтобы работать с Iceberg metadata tables.
...
def log_iceberg_files_summary(trino: TrinoClient, table_name: str) -> None:
    # Читаем системную таблицу Iceberg "$files" и логируем физическую упаковку данных.
...
def log_batch_row_count(trino: TrinoClient, table_name: str, batch_id: str) -> None:
    # Считаем только строки нужного batch, чтобы понимать размер текущей загрузки.
    batch = q(batch_id)
    row = trino.fetchone(
...
def validate_bronze_batch(trino: TrinoClient, batch_id: str) -> Dict[str, int]:
    # Проверяем, что в bronze есть все источники текущего batch.
    batch = q(batch_id)
    ...
    # Если хотя бы один источник пустой, собирать silver нельзя.
...
def ensure_silver_schema(trino: TrinoClient) -> None:
    # Физическое расположение слоя silver в MinIO.
...
def ensure_silver_tables(trino: TrinoClient) -> None:
    # Создаём все физические таблицы слоя silver из DDL.
...
def refresh_dim_article(trino: TrinoClient, batch_id: str) -> None:
    # Размерность товара обновляется как delete + insert для всех затронутых ключей.
    batch = q(batch_id)
...
def refresh_dim_customer(trino: TrinoClient, batch_id: str) -> None:
    # Клиентская размерность обновляется тем же паттерном, что и товарная.
    batch = q(batch_id)
    delete_sql = SQL_ASSETS.render(
        "sql",
        "queries",
...
def upsert_dim_date(trino: TrinoClient, batch_id: str) -> None:
    # Календарь не пересобирается целиком, а пополняется новыми датами.
    batch = q(batch_id)
...
def get_batch_months(trino: TrinoClient, batch_id: str) -> List[str]:
    # Ищем все месяцы, которые реально присутствуют в транзакциях текущего batch.
    batch = q(batch_id)
    rows = trino.fetchall(
        f"""
...
def parse_months_arg(months_arg: str | None) -> List[str]:
    # Пользователь может ограничить пересборку только частью месяцев.
    if not months_arg:
        return []
...
def resolve_months_to_process(batch_months: List[str], requested_months: List[str]) -> List[str]:
    # Если месяцы не переданы, обрабатываем весь batch.
    if not batch_months:
        return []
...
def month_filter(column_name: str, month_start: str) -> str:
    # Граница месяца строится как [start, next_month).
    return (
...
def refresh_fact_sales_line_month(
    ...
    # Для каждого месяца удаляем старые строки и вставляем новые.
    batch = q(batch_id)
...
def refresh_fact_sales_line_by_month(
    ...
    # Нормализуем месяцы batch и фильтр пользователя в один список работы.
...
def fact_sales_line_batch_exists(trino: TrinoClient, batch_id: str) -> bool:
    # Нужен для выбора безопасного сценария обновления агрегата stats.
    batch = q(batch_id)
    row = trino.fetchone(
        f"""
        SELECT 1
...
def merge_fact_customer_article_stats_batch_delta(
    ...
    # Для нового batch используем MERGE по агрегированным парам клиент-товар.
    batch = q(batch_id)
...
def get_impacted_prefixes(
    ...
    # Для повторного batch ограничиваем пересборку только затронутыми префиксами customer_id.
    batch = q(batch_id)
...
def delete_impacted_stats_prefix(
    ...
    # Сначала удаляем уже существующие пары в затронутом префиксе.
    batch = q(batch_id)
...
def insert_impacted_stats_prefix(
    ...
    # Потом пересчитываем те же пары из fact_sales_line.
    batch = q(batch_id)
    prefix = q(prefix_value)
...
def refresh_fact_customer_article_stats_incremental(
    ...
    # Собираем список префиксов и пересчитываем каждый из них отдельно.
...
def refresh_fact_customer_article_stats(
    ...
    *,
    ...
    # Выбираем сценарий обновления агрегата в зависимости от того, был ли batch уже в fact_sales_line.
...
def parse_args() -> argparse.Namespace:
    # Аргументы запуска управляют только batch и двумя операционными режимами.
...
def main() -> None:
    # Собираем весь оркестрационный сценарий по шагам.
    args = parse_args()
